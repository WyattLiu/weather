...
def score_trade(trade, portfolio_state, skip_waterfall=False):
    ...
    type_bonus = {'LET EXPIRE': 15, 'ASSIGNMENT': 10,
                  ...
                  'STRANGLE': 1, 'CLOSE': 1}.get(trade['type'], 0)
    ...
    def _recovery_days(shares, price_move_pct, daily_theta):
        """Days of theta to recover from a spot move's share P&L impact."""
        price_move = spot * price_move_pct
        share_pnl = abs(shares * price_move)  # absolute loss from the move
        # Theta also changes with spot, but approximate with current theta
        if daily_theta < 1:
            return 999
        return share_pnl / daily_theta
    ...
    over_kelly_mult = max(0, kelly_pct - 0.50) * 4  # 0.92 → 1.68
    ...
    if gamma_regime_score > 0 and trade['type'] in ('OPEN', 'ADD', 'STRANGLE'):
...
def compute_recommendations(spot, iv, expiry_groups, weekly_theta, smoothness, avg_weekly_theta, today, thesis_tilt=0.0):
    ...
    def _filter_candidates(candidates, state, used_sources, used_source_qty, used_targets, synthetic_positions):
        ...
        for c in candidates:
            ...
            _t_typ = c['type']
            if _t_typ in ('ADD', 'STRANGLE', 'OPEN', 'COVERED CALL', 'BUY PUT'):
                _income_mode_filter = (
