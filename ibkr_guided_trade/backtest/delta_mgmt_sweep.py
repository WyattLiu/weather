"""Phase 2: delta MANAGEMENT on the winning gamma formula (CURVE ×1.5). Sweep convergence pace
(cut_speed), re-eval cadence, and the hysteresis band — can better delta control close the Sharpe gap
to flat shares (2.01)?"""
...
def g15(**kw): return {**BASE, 'reaccum_delta_gamma': True, 'boxx_cash_buffer': 15000, 'reaccum_put_dte': 30,
                       'reaccum_put_moneyness': 0.05, 'gamma_target_mode': 'curve_k', 'gamma_curve_k': 1.5, **kw}
V = {
    'shares ref (2.01)':    dict(BASE),
    'CURVE1.5 base':        g15(),
    'cut0.3':               g15(cut_speed=0.3),
    'cut0.7':               g15(cut_speed=0.7),
    'cadence14':            g15(z_target_cadence_days=14),
    'cut0.3+cad14':         g15(cut_speed=0.3, z_target_cadence_days=14),
    'cut0.7+cad14':         g15(cut_speed=0.7, z_target_cadence_days=14),
}
def _load():
...
if __name__=='__main__':
    ...
    print(f"  {'delta mgmt':<20}{'win':<7}{'ann':>7}{'Sh':>6}{'MaxDD':>7}")
    for n,w,a,s,m in res: print(f"  {n:<20}{w:<7}{a:>6.1f}%{s:>6.2f}{m:>6.1f}%")
